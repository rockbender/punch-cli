...
export default class InCommand extends Command {
  ...
  static flags = {
    force: flags.boolean({
      char: "f",
      description:
        "Force re-start a new session. Discards an active session if found.",
    }),
    message: flags.string({
      char: "m",
      description: "Set note. Can only be used while creating a new session.",
    }),
    amend: flags.string({
      char: "a",
      description:
        "Amend note to the current seesion. Cannot be used when creating a new session.",
    }),
  };
  ...
  async run() {
    ...
    if (this.flags.force || currentSession == null) {
      ...
      this.log(
        chalk.green("\tNotes:\t\t"),
        chalk.green(currentSession.Notes === "" ? "-" : currentSession.Notes)
      );
    } else {
      let startDateTime = new Date(currentSession.StartDateTime);
      this.log(
        `\nSession running since, ${chalk.green(
