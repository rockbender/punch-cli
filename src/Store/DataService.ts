...
export default class DataService extends CoreDataService {
  ...
  getSessionLogs(): ISessionLog[] {
    ...
    try {
      const stmt = this.dbo.prepare(
        `SELECT * FROM ${DbTables.SessionLog.Name} order by id desc`
      );
      result = stmt.all();
    } catch (err) {
  ...
  addSession(date: Date, msg: string): void {
    ...
    try {
      ...
      stmt = this.dbo.prepare(
        `INSERT INTO ${DbTables.Session.Name} VALUES(?, ?, ?)`
      );
  ...
  updateCurrentSession(msg: string) {
    try {
      const stmt = this.dbo.prepare(
        `UPDATE ${DbTables.Session.Name} SET Notes = '${msg}'`
      );
      stmt.run();
    } catch (err) {
      console.log(
        "Error thrown while attempting to update an existing session.",
        err
      );
    }
  }
